...
//		auth.userDetailsService(inMemoryUserDetailsManager());
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.csrf().disable().anonymous().and().httpBasic().and().authorizeRequests().antMatchers("/bloodrequest").permitAll()
				.anyRequest().authenticated().and().addFilter(new JwtAuthorizationFilter(authenticationManager()));
		httpSecurity.csrf().disable().anonymous().and().httpBasic().and().authorizeRequests().antMatchers("/hospitalslot").permitAll()
		.anyRequest().authenticated().and().addFilter(new JwtAuthorizationFilter(authenticationManager()));
		httpSecurity.csrf().disable().anonymous().and().httpBasic().and().authorizeRequests().antMatchers("/donate").permitAll()
		.anyRequest().authenticated().and().addFilter(new JwtAuthorizationFilter(authenticationManager()));
		httpSecurity.csrf().disable().anonymous().and().httpBasic().and().authorizeRequests().antMatchers("/search").permitAll()
		.anyRequest().authenticated().and().addFilter(new JwtAuthorizationFilter(authenticationManager()));
